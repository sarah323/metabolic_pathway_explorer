# Metabolic Pathway Explorer

An interactive biomedical learning platform for exploring insulin regulation, metabolic homeostasis, and clinical metabolic disorders through dynamic visualization and simulation.

## Overview

**Metabolic Pathway Explorer** is an open educational software resource designed to help learners understand how changes in insulin concentration and tissue sensitivity influence intracellular signaling, metabolic pathways, biochemical markers, and disease states.

Rather than memorizing isolated pathways, learners can explore the relationship between:

- Insulin signaling activity
- PI3K–AKT pathway regulation
- GLUT4 translocation
- FOXO1-mediated gluconeogenesis
- mTORC1/SREBP-1c lipogenesis
- Hormone-sensitive lipase (HSL) regulation
- Lipoprotein lipase (LPL) activity
- Clinical biochemical changes in metabolic disorders

## Features

### Interactive Pathway Explorer

- Adjust insulin levels and insulin sensitivity
- Visualize changes in intracellular signaling activity
- Explore downstream metabolic effects

### Disease Explorer

Includes interactive cases of:

- Normal physiology
- Insulinoma
- Diabetic ketoacidosis (DKA)
- Type 2 diabetes mellitus (T2DM)

### Metabolic State Simulator

Allows learners to manipulate metabolic parameters and observe:

- Enzyme activity changes
- Glucose regulation
- Triglyceride metabolism
- Ketone body production
- Overall metabolic state
Virtual Laboratory Report

Provides simulated biochemical profiles with interpretation for different clinical scenarios.

Self-Assessment

Includes interactive questions with immediate feedback to reinforce understanding.

Educational Use

Designed for:

* Medical students
* Biomedical sciences students
* Pharmacy students
* Medical laboratory science students
* Postgraduate trainees
* Medical educators

Learning Objectives

After completing this module, learners should be able to:

* Explain insulin receptor signaling mechanisms.
* Describe PI3K–AKT pathway activation.
* Explain GLUT4 translocation and glucose uptake.
* Understand regulation of FOXO1 and hepatic glucose production.
* Explain mTORC1/SREBP-1c regulation of lipid metabolism.
* Interpret biochemical changes in metabolic diseases.
* Compare insulinoma, DKA, and type 2 diabetes mellitus.

Live Demo

Available through GitHub Pages:

https://sarah323.github.io/metabolic_pathway_explorer/

Developer

Sarah Salah, MD

Assistant Lecturer of Medical Biochemistry & Molecular Biology
Faculty of Medicine, Alexandria University

ORCID:
https://orcid.org/0009-0001-7198-0515

References

The educational content is based on established biomedical textbooks and peer-reviewed literature, including:

* Nelson & Cox. Lehninger Principles of Biochemistry.
* Rodwell et al. Harper’s Illustrated Biochemistry.
* Haeusler RA, McGraw TE, Accili D. Metabolic signalling: biochemical and cellular properties of insulin receptor signalling. Nature Reviews Molecular Cell Biology.
* Petersen MC, Shulman GI. Mechanisms of insulin action and insulin resistance. Physiological Reviews.

Citation

If you use this resource, please cite:

Salah S. (2026). Metabolic Pathway Explorer (Version 1.0.0). Interactive educational software. GitHub.

License

This project is released under the Creative Commons Attribution 4.0 International License (CC BY 4.0).
